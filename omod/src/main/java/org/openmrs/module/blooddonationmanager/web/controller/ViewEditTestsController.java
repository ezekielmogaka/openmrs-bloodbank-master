...
@Controller
@RequestMapping("/module/blooddonationmanager/viewEditTests.form")
public class ViewEditTestsController{
	...
	public String onSubmit(ModelMap model){
		return "/module/blooddonationmanager/viewEditTests";
	}
	...
	public String initList(ModelMap model) {
		...
		List<BloodDonationManager> records = service.getTestableRecords();
		List<BloodDonationManager> mapRecords = service.getTestableRecords();
		
		String donorIdientifier = Context.getAdministrationService().getGlobalProperty("blooddonationmanager.donorIdTypeId");
		
		for(BloodDonationManager bloodBank : records){
			...
			if((bloodBank.getTest()!=null
				...
				Integer.valueOf(Context.getAdministrationService().getGlobalProperty("blooddonationmanager.test.valid.count"))
			){
