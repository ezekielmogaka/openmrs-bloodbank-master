...
<openmrs:globalProperty key="blooddonationmanager.question.valid.count" defaultValue="0" var="qcount"/>
<openmrs:globalProperty key="blooddonationmanager.test.valid.count" defaultValue="0" var="tcount"/>
<openmrs:globalProperty key="blooddonationmanager.result.valid.count" defaultValue="0" var="rcount"/>
<p></p>
				...
				<div id="${count1}">
					<table cellspacing="0" cellpadding="2" id="donorData" class="stock">
						...
						<c:forEach items="${bloodUnits}" var="rec">
							...
							<td style="">${rec.blooddonationmanagerId}</td>
							<td class="encounterEdit">
								<c:if test="${ rec.bloodResult != null }">
									<openmrs:hasPrivilege privilege="Edit Encounters">
										<c:choose>
											<c:when test="${!rec.expired}">
												<!-- <a href="${pageContext.request.contextPath}/module/htmlformentry/htmlFormEntry.form?encounterId=${rec.bloodResult.encounterId}&mode=EDIT&returnUrl=${pageContext.request.contextPath}/module/blooddonationmanager/viewStock.form#${count1}">-->
											...
											<c:otherwise>
												<form action="viewStock.form#${count1}" id="disposeForm" method="post" accept-charset="utf-8">
													...
													<button type="submit" onclick="confirmDispose(${rec.blooddonationmanagerId})"><img src="${pageContext.request.contextPath}/images/trash.gif" title="Edit" align="top" border="0">Dispose</button>
												</form>
											</c:otherwise>
										</c:choose>
							...
							<td class="encounterEdit">
								<c:if test="${ rec.test != null }">
									<openmrs:hasPrivilege privilege="Edit Encounters">
										<!-- <a href="javascript:void(0)" onclick="loadUrlIntoEncounterPopup('${rec.test.encounterType.name} | ${rec.test.location.name} | <openmrs:formatDate date='${rec.test.encounterDatetime}'/> | ${rec.patient.personName}', '${pageContext.request.contextPath}/module/htmlformentry/htmlFormEntry.form?encounterId=${rec.test.encounterId}&mode=VIEW&returnUrl=${pageContext.request.contextPath}/module/blooddonationmanager/viewStock.form&amp;inPopup=true'); return false;">-->
							...
							<td class="encounterType">
								<a href="printBloodTag.form?bbId=${rec.blooddonationmanagerId}" target="none"><img src="${pageContext.request.contextPath}/images/printer.gif" title="print bloodtag" align="top" border="0"/>Print</a>
							</td>

							<td class="donorId">
								<openmrs:hasPrivilege privilege="Edit Encounters">
									<a href="${pageContext.request.contextPath}/module/blooddonationmanager/showDonorEncounters.form?patientId=${rec.patient.patientId}">${donorId[rec.patient]}</a>
								</openmrs:hasPrivilege>
							</td>
